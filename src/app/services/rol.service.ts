...
export class RolService {
  ...
  public registerAdmin(rol: Rol): Observable<any> {
    let headers = this.createTokenHeaderR();
    return this.httpClient.post(this.url + '/registerAdmin', rol, { headers: headers });
  }

  //Guardar admin
  public registerDoctor(rol: RegisterDoctor): Observable<any> {
    let headers = this.createTokenHeaderR();
    return this.httpClient.post(this.url + '/registerDoctor', rol, { headers: headers });
  }

  //Guardar admin
  public registerPatient(rol: RegisterPatient): Observable<any> {
    let headers = this.createTokenHeaderR();
    return this.httpClient.post(this.url + '/registerPatient', rol, { headers: headers });
  }
}
